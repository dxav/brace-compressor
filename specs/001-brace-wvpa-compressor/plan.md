# Implementation Plan: BRACE WVPA Codec

**Branch**: `remove-transformer`

## Design

The package exposes `BraceCodec` under `src/brace_compressor/`. Encoding extracts and separately compresses the missing mask, walks valid cells in deterministic order, quantizes residuals using the configured bound, verifies the decoder simulation, and writes a versioned container. Decoding repeats the same walk and restores the sentinel from the mask.
