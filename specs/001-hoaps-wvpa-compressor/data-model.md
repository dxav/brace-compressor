...
| Field | Type | Description |
|-------|------|-------------|
| `weights` | versioned model parameters | Deterministic random init (seed 7) is the default; optional trained weights (`MODEL_VERSION=3`) can be loaded via `load_weights()`. Version recorded in container metadata (must match at decode). |
